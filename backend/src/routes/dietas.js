...
// checkPermission diz SE pode ler dieta; este diz sobre QUAL paciente (isolamento
// entre empresas). Nas rotas por :id do plano/item o guard mora no controller,
// porque o animal só é conhecido depois de carregar o registro.
...
router.get('/planos/animal/:animalId', authenticate, checkPermission('nutricao.dietas.ler', 'LEITURA'), exigirAcessoAnimal(), PlanoDietaController.listarPorAnimal);

// Buscar planos por nome ou status — /api/dietas/planos/animal/:animalId/buscar?q=&ativo=
router.get('/planos/animal/:animalId/buscar', authenticate, checkPermission('nutricao.dietas.ler', 'LEITURA'), exigirAcessoAnimal(), PlanoDietaController.buscar);
...
// Criar plano
router.post('/planos', authenticate, checkPermission('nutricao.dietas.criar', 'PROPRIO'), exigirAcessoAnimal({ de: 'body' }), PlanoDietaController.criar);
...
router.get('/animal/:animalId', authenticate, checkPermission('nutricao.dietas.ler', 'LEITURA'), exigirAcessoAnimal(), DietaItemController.listarPorAnimal);
...
// Criar item
router.post('/', authenticate, checkPermission('nutricao.dietas.criar', 'PROPRIO'), exigirAcessoAnimal({ de: 'body' }), DietaItemController.criarItem);
