// frontend/src/components/farmacia/LeitorDocumentoCompra.tsx
//
// LEITURA DO DOCUMENTO DE COMPRA do fornecedor (2026-09-10; DE VOLTA na Farmácia em
// 2026-09-18).
//
// ⚠️ Ele nasceu na tela de PRODUTOS e foi removido em 2026-09-15, quando aquela tela
// passou a cadastrar o ITEM e não a compra. Voltou AQUI, que é onde a compra de fato
// acontece: a Entrada de Estoque. O backend nunca foi apagado — o que mudou foi a
// rota que este componente chama (`/farmacia/estoque/documento-compra`).
//
// 🔴 UMA NOTA TRAZ VÁRIOS PRODUTOS, e é isso que a lista com checkbox resolve: cada
// item lido vira uma ENTRADA DE ESTOQUE própria. Todos nascem marcados — quem abriu a
// nota quer dar entrada no que ela traz, e desmarcar o que não interessa é menos
// trabalho do que marcar item a item.
//
// 🔴 SERVE ÀS DUAS TELAS DE ESTOQUE — Farmácia e Vacinas (2026-09-19). O que muda é
// a ROTA (o gate de cada módulo) e o TIPO de item que aquela tela dá entrada; tudo o
// mais é idêntico. Uma cópia por tela divergiria na primeira correção, e o que
// divergiria é o que a pessoa entende por "o que a nota trouxe" (armadilha 28-g).
//
// 🔴 O CRITÉRIO CONTINUA SENDO "DOCUMENTO DE COMPRA", NÃO "DOCUMENTO FISCAL". Vale
// nota fiscal, cupom, ORÇAMENTO DE BALCÃO e recibo — o balcão do fornecedor
// veterinário entrega o tempo todo papel com "SEM VALOR FISCAL" impresso, e ele traz
// emitente, itens, quantidade e preço, que é tudo o que esta tela precisa. Validade
// fiscal nunca foi requisito: daqui sai catálogo e preço de compra, não escrituração
// contábil — o backend (`ai/prompts/lerNotaFiscal.js`) não mudou.
// ⚠️ O RÓTULO passou a ser "Carregar Nota Fiscal" (a pedido, 2026-09-19), revertendo
// o nome ampliado de 2026-09-10. A razão daquele nome CONTINUA de pé: enquanto o
// rótulo dizia só "nota fiscal", quem estava com um orçamento na mão não tentava.
// Por isso o texto de apoio e o `title` do botão seguem dizendo, com todas as letras,
// que cupom, orçamento e recibo são aceitos — o que mudou foi o NOME, não a função.
// Não apagar essas frases junto com o título.
//
...
interface Props {
  ...
  /**
   * Endpoint que lê a nota. Cada módulo tem o seu, porque cada um tem o seu GATE:
   * farmácia exige `farmacia.estoque.criar`, vacina exige `vacina.estoque.criar`.
   * O controller do outro lado é o MESMO (`NotaFiscalController.ler`).
   */
  rota?:    string;
  /**
   * O tipo que ESTA tela dá entrada. Os itens dele nascem MARCADOS; os do outro tipo
   * aparecem DESMARCADOS, com aviso.
   * ⚠️ Eles NÃO são escondidos, de propósito: a nota do fornecedor veterinário mistura
   * vacina e medicamento, e sumir com metade dela faria a pessoa concluir que a
   * leitura falhou. Além disso a classificação é da IA e pode errar — desmarcado, o
   * erro custa um clique; escondido, custa redigitar o item.
   */
  tipoItem?: 'medicamento' | 'vacina';
}

export default function LeitorDocumentoCompra({
  aberto, onFechar, onUsar,
  rota = '/farmacia/estoque/documento-compra',
  tipoItem,
}: Props) {
  ...
  const escolherArquivo = async (file: File | null) => {
    ...
    try {
      ...
      setNota(lida);
      // Marcados por padrão: quem abriu a nota quer cadastrar o que ela traz, e
      // desmarcar o que não interessa é menos trabalho que marcar item a item.
      // ⚠️ Com `tipoItem`, só os DAQUELE tipo nascem marcados — esta tela dá entrada
      // de um tipo só, e marcar o outro levaria a pessoa a criar, sem perceber, um
      // lote de vacina para um medicamento (ou o contrário).
      setMarcados(new Set(
        lida.itens.map((it, i) => (!tipoItem || it.tipo === tipoItem ? i : -1)).filter(i => i >= 0),
      ));
    } catch (err) {
      const e = err as { isPermissionError?: boolean; code?: string; response?: { status?: number; data?: { error?: string } }; message?: string };
      if (e.response?.status === 429) {
        setErro('O limite de uso de IA do plano foi atingido. Fale com o responsável pela clínica.');
      } else if (!e.isPermissionError) {
        // ⚠️ "timeout of 180000ms exceeded" é o texto da BIBLIOTECA: ele não diz o que
        // fazer, e o que a pessoa faria por conta própria — refazer a foto — é sobre o
        // que estava certo. Mesma razão do `motivoDaFalha` do backend.
        const demorou = e.code === 'ECONNABORTED' || /timeout/i.test(e.message ?? '');
        setErro(demorou
          ? 'A leitura demorou mais que o esperado e foi interrompida. O serviço pode estar sobrecarregado — tente de novo em instantes.'
          : e.response?.data?.error ?? e.message ?? 'Erro ao ler o documento.');
      }
    } finally {
      setLendo(false);
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl border border-gray-100 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 rounded-t-2xl">
          <h3 className="font-bold text-gray-900 flex items-center gap-2">
            <FileText size={18} className="text-emerald-600" /> Carregar Nota Fiscal
          </h3>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          {!nota && (
            <>
              {/* ⚠️ O título diz "Nota Fiscal", mas o que é ACEITO é mais que isso — e
                  precisa estar escrito aqui, senão quem tem um orçamento de balcão na
                  mão nem tenta (a razão do nome antigo, 2026-09-10). */}
            ...
            <>
              ...
              <div className={`rounded-xl border px-3 py-2.5 ${
                ...
                <p className={`text-[11px] mt-0.5 ${nota.fornecedorExistente ? 'text-emerald-700' : 'text-amber-800'}`}>
                  {nota.fornecedorExistente
                    ? 'Ele será selecionado automaticamente na entrada de estoque.'
                    : 'Cadastre-o em Cadastro › Fornecedores para que a compra entre na conta a pagar dele.'}
                </p>
              ...
              {/* Itens do OUTRO tipo: ficam à vista e desmarcados. A nota do balcão
                  veterinário mistura vacina e medicamento, e cada tela dá entrada de um
                  tipo só. */}
              {tipoItem && nota.itens.some(it => it.tipo !== tipoItem) && (
                <div className="flex items-start gap-2 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5">
                  <AlertTriangle size={15} className="text-gray-400 flex-shrink-0 mt-0.5" />
                  <p className="text-[11px] text-gray-600 leading-snug">
                    A nota traz {nota.itens.filter(it => it.tipo !== tipoItem).length}{' '}
                    {tipoItem === 'vacina' ? 'item que não é vacina' : 'item que é vacina'} —{' '}
                    <strong>desmarcado</strong>, porque esta tela dá entrada de{' '}
                    {tipoItem === 'vacina' ? 'vacinas' : 'medicamentos'}. Se a classificação
                    estiver errada, marque assim mesmo.
                  </p>
                </div>
              )}
