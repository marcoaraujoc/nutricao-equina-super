...
import {
  ...
  Lock, Globe, Pencil, Ban, Mail, ChevronDown, CalendarDays, Syringe, Package,
...
const CARGO_INFO: Record<string, { label: string; desc: string; cor: string; tipo?: string }> = {
  ...
  FORNECEDOR:   { label: 'Fornecedor',   desc: 'Fornecedor de produtos/serviços com login. Acesso configurável pelo gestor da equipe.',      cor: 'teal',    tipo: 'SISTEMA' },
  // Cargo novo (2026-09-09), irmão do FORNECEDOR — mesmo comportamento, cadastro em
  // Prestadores. Nada foi migrado: quem já era FORNECEDOR continua sendo.
  PRESTADOR:    { label: 'Prestador',    desc: 'Prestador de serviços externo (ferrador, fisioterapeuta…). Acesso configurável pelo gestor da equipe.', cor: 'teal', tipo: 'SISTEMA' },
...
// ═══════════════════════════════════════════════════════════════════════════════
// ABA — Equipe (membros profissionais: Veterinário, Estagiário, Enfermeiro,
// Secretaria, Financeiro)
//
// 🔴 FORNECEDOR e PRESTADOR NÃO ENTRAM AQUI desde 2026-09-09: são atuações
// ESTANQUES, não equipe. Eles são criados e geridos nos próprios cadastros
// (`/cadastro/fornecedores`, `/cadastro/prestadores`), e é de lá que sai também a
// designação de pacientes. O backend já não os devolve nesta lista
// (`lib/cargosPrestador.js#SEM_EXTERNOS`).
// ═══════════════════════════════════════════════════════════════════════════════
...
function TabEquipe({ equipeId, isGestor }: { equipeId: number; isGestor: boolean }) {
  ...
  const handleIncluirModal = async (values: UsuarioFormValues) => {
    ...
    setErroModal(null);
    try {
      // 🔴 SÓ CONVITE AQUI (2026-09-09). O ramo de INCLUSÃO DIRETA existia para
      // FORNECEDOR/PRESTADOR — que deixaram de ser equipe e agora nascem nos
      // próprios cadastros (`/cadastro/fornecedores`, `/cadastro/prestadores`),
      // onde também se concede o acesso e se designam os pacientes.
      {
  ...
  // Os DOIS cargos externos têm escopo por designação — a coluna vale para ambos.
